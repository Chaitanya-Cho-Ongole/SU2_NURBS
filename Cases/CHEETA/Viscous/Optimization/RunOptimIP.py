...
dragAdjointMaster="DRAG_ADJOINT.cfg"
...
# drag adjoint analysis -> AD + projection
drag_adjoint_command = "mpirun -n " + ncores + " SU2_CFD_AD " + dragAdjointMaster + " && mpirun -n " + ncores + " SU2_DOT_AD " + geoMasterFuse
...
# DRAG ADJOINT ANALYSIS---------------------------------------------------#
def make_drag_AdjRun(name, func=None) :
    adj = ExternalRun(name,drag_adjoint_command,True)
    adj.setMaxTries(max_tries)
    adj.addConfig(dragAdjointMaster)
...
# Drag adjoint
drag_adj = make_drag_AdjRun("DRAG_ADJ",func_drag)
